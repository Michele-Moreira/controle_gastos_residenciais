...
Commits deste projeto seguem rules fixas: Conventional Commits, subject todo em minúsculo, escopo obrigatório e ≤72 chars. Nunca incluir `Co-authored-by` ou menção a IA.

Steps: construir subject, mostrar ao usuário e commitar (sem dar push).
---
...
- **Sem ponto final**.
- **Corpo opcional**, só quando o _porquê_ não for óbvio pelo diff. Explique motivação/contexto, não o que o diff já mostra.
