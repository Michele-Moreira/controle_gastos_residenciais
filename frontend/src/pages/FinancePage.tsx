...
import { criarPessoa, criarTransacao, excluirPessoa, listarPessoas, listarTotais, listarTransacoes } from "../lib/api";
...
function FinancePage() {
	...
	const carregarDados = useCallback(async () => {
		try {
			const [pessoasResponse, transacoesResponse, totaisResponse] = await Promise.all([
				listarPessoas(),
				listarTransacoes(),
				listarTotais(),
			]);
	...
	const renderPessoaTotais = (pessoa: TotaisPessoa) => (
		<li key={pessoa.pessoaId} className="totais-item">
			<div>
				...
				<p className="totais-subtitle">Receitas: R$ {pessoa.totalReceitas.toFixed(2)}</p>
				<p className="totais-subtitle">Despesas: R$ {pessoa.totalDespesas.toFixed(2)}</p>
			</div>
		...
		<div className="app-shell">
			...
			<section className="panel panel-wide">
				...
				{totais?.pessoas && totais.pessoas.length > 0 ? (
					<ul className="totais-grid">{totais.pessoas.map(renderPessoaTotais)}</ul>
				) : (
					...
					<ul className="list">
						{pessoas.map((pessoa) => (
							<li key={pessoa.id}>
								...
								<button type="button" onClick={() => handleExcluirPessoa(pessoa.id)}>
									Excluir
								</button>
							</li>
						))}
